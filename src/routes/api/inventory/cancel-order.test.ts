...
import { Route } from "./cancel-order";
...
vi.mock("@/lib/inventory/orderReservation", () => ({ cancelOrderReservation: vi.fn() }));

function request(body: unknown, headers: Record<string, string> = {}) {
  return new Request("http://localhost/api/inventory/cancel-order", {
    method: "POST",
...
beforeEach(() => {
  ...
  vi.mocked(cancelOrderReservation).mockReset();
});

describe("POST /api/inventory/cancel-order", () => {
  it("rejects an unauthorized caller with 401", async () => {
    ...
    expect(cancelOrderReservation).not.toHaveBeenCalled();
  });
  ...
  it("maps success to 200, invalid_state to 422, not_found to 404", async () => {
    ...
    vi.mocked(cancelOrderReservation).mockResolvedValue({
      status: "success",
